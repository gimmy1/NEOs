...
class NearEarthObject:
    ...
    def __str__(self):
        """Return `str(self)`."""
        return f"An NEO encapsulates semantic and physical parameters about the object, such \
                 as its primary designation (required, unique), IAU name (optional), diameter \
                 in kilometers (optional - sometimes unknown), and whether it's marked as  \
                 potentially hazardous to Earth, and a collection of close approaches"
...
class CloseApproach:
    ...
    def __init__(self, designation, time=None, distance=float(0.0), velocity=float(0.0)):
        ...
        try:
            ...
            logger.error(f"Exception occurred: {te}")
        finally:
            self.time = time
            logger.info("Successfully converted time")
